...
grinder -reference_file $genomes/Wuhan-Hu-1_SARS-COV-2.fasta -forward_reverse $input/V4_POS$i\.fas -coverage_fold $y -read_dist $x -insert_dist 350 -mate_orientation FR -mutation_dist linear 0.1 0.2 -qual_levels 30 10 -fastq_output 1 -output_dir $output/ -base_name Wh1_POS$i\_PE$x\_$y\X_IE_$z
done
done
done
done
...
do
echo "x: $x, y: $y, z: $z"
cat $output/Wh1_POS*_PE$x\_$y\X_IE_$z\-reads.fastq > $final_output/Wh1_V4_PE$x\_$y\X_IE_$z\-reads.fastq
done
done
done