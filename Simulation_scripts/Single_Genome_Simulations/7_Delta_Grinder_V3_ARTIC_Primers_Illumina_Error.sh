...
output='/scratch/mandyh/WISER_InSilico/Amplicon/sim_V3_Delta_Illumina_reads_error'
...
grinder -reference_file $genomes/Delta_gisaid_hcov-19_GA_2021_06_30_14.fasta -forward_reverse $input/V3_POS$i\.fas -coverage_fold $y -read_dist $x -insert_dist 350 -mate_orientation FR -mutation_dist linear 0.1 0.2 -qual_levels 30 10 -fastq_output 1 -output_dir $output/ -base_name Delta_POS$i\_PE$x\_$y\X_IE_$z
done
done
done
done
...
do
echo "x: $x, y: $y, z: $z"
cat $output/Delta_POS*_PE$x\_$y\X_IE_$z\-reads.fastq > $final_output/Delta_V3_PE$x\_$y\X_IE_$z\-reads.fastq
done
done
done